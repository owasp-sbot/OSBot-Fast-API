...
*by Dinis Cruz, ChatGPT Deep Research and Claude Opus 4.1, 27th September 2025, v0.27.0*
...
extensions][https://github.com/owasp-sbot/OSBot-Fast-API]) and outline how to design its client
...
-   **Client Class and Methods:** The client library provides a class
    (e.g. `Cache__Client`) with methods for each major operation of the
...
We will maintain **separate repositories** for the service and its client library. For example, for the
Cache service:
...
```
    ...
    │   ├── client.py               # Main client class (Cache_Client)  
    │   ├── endpoints.py            # auto-generated low-level endpoint mapping/functions  
    │   └── utils.py                # utilities (e.g., for local mode or auth)  
    └── tests/  
...
`Cache_Item`, `Cache_Store_Request`, `Cache_Store_Response`, etc., all
subclassing `Type_Safe`. The `client.py` would define `Cache_Client` with
...
**Shared Schema Example:** If `Cache_Item` is a Type_Safe class with
...
```python
    # In service code
    from mgraph_ai_cache_client.schemas.cache_models import Cache_Item, Cache_Store_Request

    class Routes_Cache(Fast_API__Routes):
        tag = "cache"

        def store_json(self, request: Cache_Store_Request) -> Cache_Item:
            # ... (store logic) ...
            return Cache_Item(id=stored_id, data=request.data)
        
        def get_item__id(self, id: Safe_Id) -> Cache_Item:
            # ... retrieve item by id ...
            return Cache_Item(id=id, data=...)
...
In the above, the schemas `Cache_Store_Request` and `Cache_Item` come from
...
```python
    ...
    class Cache_Client:
        ...
        def store_json(self, data: dict, strategy: str = "default", namespace: str = "default") -> cache_models.Cache_Item:
            """Store a JSON payload in the cache and return a Cache_Item handle."""
            ...
            return cache_models.Cache_Item.from_json(response.json())
```
...
create a `Cache_Item` object using Type_Safe's `from_json()` method. 
...
receives a fully-typed `Cache_Item` instance ready to use.
...
```python
        def get_item(self, item_id: Safe_Id, namespace: Safe_Str__Namespace = None) -> cache_models.Cache_Item:
        ...
        def search_items(self, query_params: cache_models.SearchParams) -> List[cache_models.Cache_Item]:
            ...
            return [cache_models.Cache_Item.from_json(item) for item in items_json]
```
...
```python
    ...
    class Cache_Metadata(Type_Safe):
...
```python
    ...
    class Cache_Client:
        def store_with_ttl(self, data     : dict               ,
                                 key      : Safe_Str__Key      ,
...
```python
    ...
    class Cache_Strategy(str, Enum):
        DIRECT          = "direct"
        TEMPORAL        = "temporal"
        TEMPORAL_LATEST = "temporal_latest"
        SEMANTIC_FILE   = "semantic_file"

    class Cache_Request(Type_Safe):
        strategy: Cache_Strategy                   = Cache_Strategy.TEMPORAL_LATEST
        priority: Literal["low", "normal", "high"] = "normal"
        
    # Client method can accept enum or string
    def store(self, data: dict, strategy: Union[Cache_Strategy, str]) -> Cache_Item:
        request = Cache_Request(strategy=strategy)  # Auto-converts string to enum
        # request.json() serializes enum to string for API
        # Cache_Request.from_json() restores enum from string
```
...
```python
    ...
    original = Cache_Request(strategy = Cache_Strategy.SEMANTIC_FILE                 ,
                             priority = "high"                                       ,
                             metadata = Cache_Metadata(tags=["important", "archive"]))
    ...
    restored = Cache_Request.from_json(json_data)
    assert restored.strategy == Cache_Strategy.SEMANTIC_FILE    # Enum restored
...
```python
    ...
    from osbot_fast_api.utils.Fast_API_Server   import Fast_API_Server
    from mgraph_ai_cache_client                 import Cache_Client, schemas
    ...
    class test_integration(TestCase):
        ...
        def test_store_and_retrieve(self):
            with Cache_Client(app=self.app) as client:  # Context manager pattern
                data    = {"foo": "bar"}
                result  = client.store_json(data)
                
                # Use Type_Safe testing patterns
                assert type(result) is schemas.Cache_Item
                assert result.obj() == __(id        = result.id,  # Use __SKIP__ for dynamic values if needed
                                          data      = data     ,
                                          namespace = "default")
...
class test_Cache_Client(TestCase):
    ...
    def test_remote_mode(self):
        ...
        with Cache_Client(base_url=os.getenv("CACHE_SERVICE_URL")) as client:
    ...
    def test_in_memory_mode(self):
        ...
        with Cache_Client(app=fast_api.app()) as client:
            result = client.store_json({"test": "data"})
            assert result.obj() == __(id        =__SKIP__         ,  # Dynamic ID
                                      data      = __(test= "data"),
                                      namespace = "default")
    ...
    def test_local_server_mode(self):
        ...
        with Fast_API_Server(app=fast_api.app()) as server:
            with Cache_Client(server=server) as client:
