...
class test_Fast_API__Contract__Extractor(TestCase):
    ...
    def test_extract_contract(self):                                                # Test contract extraction from Fast_API
        with self.extractor as _:
            ...
            assert contract.service_name      == 'Test_API'
            assert contract.version           == 'v1.0.0'
            assert contract.base_path         == '/'
            assert contract.service_version   == 'v1.0.0'
            ...
            assert contract_obj.service_name    == 'Test_API'
            assert contract_obj.version         == 'v1.0.0'
    ...
    def test__extract_module_name(self):                                            # Test module name extraction
        with self.extractor as _:
            ...
            assert _._extract_module_name('/admin/users', 'get_user')     == 'admin'
            assert _._extract_module_name('/file/upload', 'upload')       == 'file'
            assert _._extract_module_name('/data/process', 'process')     == 'data'
            assert _._extract_module_name('/api/v1/users', 'get_users')   == 'root'  # Not a known module

            # Test method-based extraction (when Routes__ pattern exists)
            assert _._extract_module_name('/', 'Routes__Users.get_user')  == 'root'
            assert _._extract_module_name('/unknown', 'some_method')      == 'root'  # Default fallback

    def test__extract_route_class(self):                                            # Test route class extraction
        with self.extractor as _:
            # For actual routes in our test Fast_API
            for route_obj in self.fast_api.app().routes:
                if hasattr(route_obj, 'endpoint'):
                    ...
                    elif 'get_product' in method_name:
                        assert route_class == 'Routes__Products'

    def test__extract_endpoint_contract(self):                                      # Test endpoint contract extraction
        with self.extractor as _:
            # Create sample route data
            route_data = { 'method_name' : 'get_user__user_id'  ,
            ...
            assert endpoint.path_pattern   == '/users/{user_id}'
            assert endpoint.method         == Enum__Http__Method.GET
            assert endpoint.route_method   == 'get_user__user_id'
            assert len(endpoint.path_params) == 1                                   # user_id parameter
            assert endpoint.path_params[0].name == 'user_id'

    def test__enhance_with_signature(self):                                         # Test signature enhancement
        with self.extractor as _:
            endpoint = Schema__Endpoint__Contract(
                operation_id = 'test_method',
    ...
    def test__enhance_with_ast_analysis(self):                                      # Test AST analysis for error codes
        with self.extractor as _:
            endpoint = Schema__Endpoint__Contract(
                operation_id = 'test_method',
                path_pattern = '/test',
            ...
            if endpoint.error_codes:
                assert 404 in endpoint.error_codes
    ...
    def test_extract_contract__with_empty_fast_api(self):                          # Test with empty Fast_API
        with Fast_API() as empty_api:
            ...
            # Even empty API has some structure
