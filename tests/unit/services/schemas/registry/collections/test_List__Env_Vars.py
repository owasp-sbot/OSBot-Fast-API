...
class test_List__Env_Vars(TestCase):

    def test__init__(self):                                                     # Test auto-initialization
        with List__Env_Vars() as _:
            assert type(_)              is List__Env_Vars
    ...
    def test__append_valid_item(self):                                          # Test appending valid items
        env_vars = List__Env_Vars()
        env_var  = Schema__Env_Var(name='TEST_VAR', required=True)
    ...
    def test__init_with_list(self):                                             # Test initialization with list
        env_vars = List__Env_Vars([Schema__Env_Var(name='VAR_1', required=True ),
                                   Schema__Env_Var(name='VAR_2', required=False)])
